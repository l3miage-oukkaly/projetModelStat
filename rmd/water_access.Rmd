...
waterAccess <- merge(waterAccess, population[, c("Country", "Year", "Population")], by = c("Country", "Year"))
waterAccess$`Water withdrawal prop (%)` <- waterAccess$`Water withdrawal per capita ()` / waterAccess$Population *100
...
```
connaitre le nom des  pays que l'on observera pas
```{r}
