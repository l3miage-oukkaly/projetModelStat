...
annualdeath$`Cardiovascular diseases prop (%)` <- annualdeath$`Cardiovascular diseases` / annualdeath$Deaths
annualdeath$`Neoplasms prop (%)` <- annualdeath$Neoplasms / annualdeath$Deaths
annualdeath$`Diabetes mellitus prop (%)` <- annualdeath$`Diabetes mellitus` / annualdeath$Deaths
annualdeath$`Respiratory diseases prop (%)` <- annualdeath$`Respiratory diseases` /  annualdeath$Deaths
annualdeath$`Digestives diseases prop (%)` <- annualdeath$`Digestives diseases` /  annualdeath$Deaths
annualdeath$`Infectious diseases prop (%)` <- annualdeath$`Infectious diseases` /  annualdeath$Deaths
annualdeath$`Other non communicable diseases (%)` <- annualdeath$`Other non communicable diseases` /  annualdeath$Deaths 
```
...
pays 
```{r}
pays_min_gov_exp <- filter(governmentEducationExpenditure, Country %in% c("Monaco", "Sudan", "Dominican Republic" , "Central African Republic","Cambodia"))

pays_max_gov_exp <- filter(governmentEducationExpenditure, Country %in% c("Cuba","Denmark",  "Djibouti", "Lesotho","Norway"))

pays_combinés_gov_exp <- filter(governmentEducationExpenditure, Country %in% c("Cuba","Denmark",  "Djibouti", "Lesotho","Norway","Monaco", "Sudan", "Dominican Republic" , "Central African Republic","Cambodia"))

pays_h1_death <- select(annualdeath,Country,Year,`Cardiovascular diseases`) %>% filter(Country %in% c("Cuba","Denmark",  "Djibouti", "Lesotho","Norway","Monaco", "Sudan", "Dominican Republic" , "Central African Republic","Cambodia"))  
...
```
```{r}
ggplot(data = pays_h1_death, aes(x = Year, y = `Cardiovascular diseases`, group = Country, color = Country)) +
  geom_line() +
  labs(title = "Cardiovascular diseases",
       x = "Country",
       y = "Cardiovascular diseases") +
  theme_minimal()+
scale_color_manual(values = alpha(hue_pal()(length(unique(pays_h1_death$Country))), 0.8))
