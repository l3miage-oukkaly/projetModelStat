...
```{r}
ggplot(governmentEducationExpenditure_minmax, aes(x= Country, y= `Mean_GPD_spent`)) + geom_bar(stat = "identity") + labs(title="Education Expenditure compared to total GDP", x="Pays", y="mean GPD spent in Education (%)") + scale_y_continuous(limits = c(0, NA), breaks = pretty_breaks(n = 10), labels = comma_format()) + scale_fill_discrete(name = "Country") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  
```
```{r}
ggplot(death_minmax, aes(x= Country, y= `Mean_death_ID`)) + geom_bar(stat = "identity") + labs(title="Infectious diseases per total number of death(10 countries education exp", x="Pays", y="mean death by Infectious diseases (%)") +  scale_y_continuous(limits = c(0, NA), breaks = pretty_breaks(n = 10),labels = comma_format()) + scale_fill_discrete(name = "Country") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
```
...
pays_combinés_enrolment_ratio <- filter(enrolment_ratio_teritiary_education, Country %in% enrolmentratio_minmax$Country)

print(enrolmentratio_minmax$Country)
...
ggplot(data = pays_h2_death, aes(x = Year, y = `Infectious diseases prop (%)`, group = Country, color = Country)) +
  geom_line() +
  labs(title = "Infectious diseases per total number of death",
       x = "Year",
...
pays_combinés_waterAcess <- filter(waterAccess, Country %in% waterAcess_minmax$Country)

print(waterAcess_valmin$Country)
print(waterAcess_valmax$Country)


pays_h3a_death <- select(annualdeath,Country,Year,`Infectious diseases prop (%)`) %>% filter(Country %in% c("Denmark","Monaco","United Kingdom","Cyprus","Sweden" )) 

pays_h3b_death <- select(annualdeath,Country,Year,`Infectious diseases prop (%)`) %>% filter(Country %in% c("Estonia","Hungary","Japan" ,"Libya","Spain"))
